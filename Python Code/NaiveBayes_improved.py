...
class NaiveBayesClassifier(object):
    ...
    def train(self, training_sentences, training_labels):
        ...
        for index in range(N_sentences):
            x = [0]*len(self.V);
            for w in range(len(training_set[index])):
                x[self.V.get(training_set[index][w])] = 1;
            BOW[index] = x;
    
        #prior
        #P(Y=c)
        prior = [0]*len(all_classes);
        for c in all_classes:
            c_count = sum(c == training_labels)+1;
            prior[c] = c_count/N_sentences;
        ...
        #P(x[d]|Y=c)
        # P(x[d],Y=c)/P(Y=c)
        conditional = np.zeros((len(all_classes), len(self.V)));
        ...
        for c in all_classes:
             ...
             for d in range(len(self.V)):
                    ...
                    for index in range(len(BOW)):
                        if(BOW[index][d] == 1 and c == training_labels[index]):
                            joint_count += 1;

                        
                    conditional[c][d] = joint_count/c_count;
    ...
    def predict(self, test_sentence):
        ...
        #binary BOW representation
        x = [0]*len(self.V);
        for w in range(len(test_sentence)):
            if(not self.V.get(test_sentence[w]) == None):
                x[self.V.get(test_sentence[w])] = 1;
            
            
        
        #log probability
        for c in all_classes:
            logPrior = np.log(self.prior[c]);
            sumLC = 0;
            for d in range(len(self.V)):
               if(x[d] == 1):
                    logConditional = np.log(self.conditional[c][d]);
               else:
                    logConditional = np.log(1-self.conditional[c][d]);
               
               sumLC += logConditional; 
            label_probability[c] = logPrior + sumLC;
